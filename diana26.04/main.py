...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.label = None
        # menu frame
        self.menu_frame = CTkFrame(self, width=30, height=300, fg_color="#200B31")
        self.menu_frame.pack_propagate(False)
        self.menu_frame.place(x=0, y=0)
        self.is_show_menu = False
        self.speed_animate_menu = -5
        self.btn = CTkButton(self, text="▶️", command=self.toggle_show_menu, width=30, fg_color="#8C3264", )
        ...
        self.send_button = CTkButton(self, text=">", fg_color="#8C3264", hover_color="#3D0D35", width=50, height=40, command=self.send_message)  # add send comm⬅️
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.speed_animate_menu *= -1
            self.btn.configure(text="◀️", hover_color="#3D0D35")
            self.show_menu()
            # setting menu widgets
            self.label = CTkLabel(self.menu_frame, text="Імʼя", fg_color="#8C3264", corner_radius=10, text_color="white")
            self.label.pack(pady=30)
            self.entry = CTkEntry(self.menu_frame)
            self.entry.pack()
            self.save_name_button = CTkButton(self.menu_frame, text="Зберегти",
                                              command=self.save_name,
                                              fg_color="#b5838d", hover_color="#e5989b",
                                              corner_radius=10,
                                              text_color="white")
    ...
    def add_message(self, text):
        bubble = CTkFrame(self.chat_field, fg_color="#3D0D35", border_width = 4, border_color="#541033")
